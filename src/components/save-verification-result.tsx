...
export default function SaveVerificationResult({
  ...
  const handleSaveComplete = () => {
    // After saving, call the callback with success=true
    onComplete(true)
  }
      ...
      <div className="w-full">
        {step === "select" && (
          <div>
            ...
            <div className="flex justify-center gap-4 mt-6">
              <Button variant="outline" onClick={onSkip || onBack} className={buttonClass}>
                {onSkip ? "Přeskočit" : "Zpět"}
              </Button>
              <Button onClick={() => onComplete(true)} style={{ backgroundColor: primaryColor }} className={buttonClass}>
                Dokončit
              </Button>
            </div>
          </div>
        ...
        {step === "result" && (
          <div className="text-center py-6">
            ...
            <Button onClick={() => onComplete(true)} className={`bg-green-600 hover:bg-green-700 ${buttonClass}`}>
              Dokončit
            </Button>
          </div>
        )}
