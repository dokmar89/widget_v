...
import FaceScanVerification from "./face-scan-verification"
import OCRVerification from "./ocr-verification"
import ReVerification from "./re-verification"
...
export default function AgeVerificationModal({
  ...
  useEffect(() => {
    if (isOpen) {
      const fetchPrices = async () => {
        try {
          const response = await ApiService.getVerificationPrices();
  ...
  const handleMethodClick = async (methodId: string) => {
    try {
      ...
      const sessionResponse = await ApiService.createSession(methodId, shopId);
      
      if (!sessionResponse.success || !sessionResponse.sessionId) {
        setError(sessionResponse.message || 'Failed to create verification session');
        setIsLoading(false);
        return;
      }
      ...
      // Pro metody, které nejsou implementovány v UI, volat API přímo
      if (methodId !== "facescan" && methodId !== "ocr" && methodId !== "reverification" && methodId !== "qrcode") {
        let verificationResponse;
        
        switch (methodId) {
          case "bankid":
            verificationResponse = await ApiService.verifyWithBankId(sessionId);
            break;
          case "mojeid":
            verificationResponse = await ApiService.verifyWithMojeId(sessionId);
            break;
          default:
            setError('Unknown verification method');
            setIsLoading(false);
            return;
        }
  ...
  const handleVerificationComplete = (success: boolean) => {
    if (success) {
      ...
      if (activeMethod) {
        ...
        onVerificationSelected(activeMethod as string)
      }
    } else {
